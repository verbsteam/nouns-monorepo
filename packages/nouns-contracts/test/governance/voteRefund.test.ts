...
const REFUND_ERROR_MARGIN = ethers.utils.parseEther('0.0012');
...
describe('V3 Vote Refund', () => {
  ...
  describe('castRefundableVote', () => {
    it('refunds users with votes', async () => {
      ...
      const tx = await gov
        .connect(user)
        ['castRefundableVote(uint256[],uint256,uint8)']([0, 1], 1, 1, DEFAULT_GAS_OPTIONS);
      ...
      await expect(tx)
        ...
        .withArgs(user.address, [0, 1], BigNumber.from(1), 1, 2, '');
    });
    ...
    it('does not refund users with no votes', async () => {
      ...
      const tx = await gov
        .connect(user2)
        ['castRefundableVote(uint256[],uint256,uint8)']([], 1, 1, DEFAULT_GAS_OPTIONS);
      const r = await tx.wait();
    ...
    it('caps refund', async () => {
      ...
      const tx = await gov
        .connect(user)
        ['castRefundableVote(uint256[],uint256,uint8)']([0, 1], 1, 1, {
  ...
  describe('castRefundableVoteWithReason', () => {
    it('refunds users with votes', async () => {
      ...
      const tx = await gov
        .connect(user)
        ['castRefundableVoteWithReason(uint256[],uint256,uint8,string)'](
          [0, 1],
          1,
          1,
          'some reason',
      ...
      await expect(tx)
        .to.emit(govWithEvents, 'VoteCast')
        .withArgs(user.address, [0, 1], BigNumber.from(1), 1, 2, 'some reason');
    });
    ...
    it('caps gasUsed', async () => {
      ...
      await expect(tx)
        .to.emit(govWithEvents, 'VoteCast')
        .withArgs(user.address, [0, 1], BigNumber.from(1), 1, 2, LONG_REASON);
    });
    ...
    it('refunds EOA user when voting via a smart contract (tx.origin)', async () => {
      ...
      const voter = await new Voter__factory(deployer).deploy(gov.address, 2, 1, true, [0]);
      await token.connect(user).transferFrom(user.address, voter.address, 0);
      await token.connect(deployer).mint();
      await token.connect(deployer).transferFrom(deployer.address, user2.address, 2);
      await advanceBlocks(1);
      ...
      await expect(tx)
        .to.emit(govWithEvents, 'VoteCast')
        .withArgs(voter.address, [0], BigNumber.from(2), 1, 1, 'some reason');
    });
  });
